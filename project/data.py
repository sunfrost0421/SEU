...
def count_yolo_data(path, classList):
    ...
    test_dir = {}
    test_count = [0 for _ in range(len(classList))]
    for file in test_labels:
        with open(test_path + "/" + file) as o:
            for line in o:
                index = int(line.split()[0])
                test_count[index] += 1
    for i in range(len(classList)):
        test_dir[classList[i]] = test_count[i]
    print(f"test_dir:{test_dir}")


data_path = "C:\\Users\\qrc\\Desktop\\newdata"
classList = ['trans', 'bridge', 'res1', 'triode', 'gnd', 'power1', 'res2', 'cap1', 'diode', 'cap2', 'mos', 'switch',
             'amplifier', 'inductance', 'power2']
